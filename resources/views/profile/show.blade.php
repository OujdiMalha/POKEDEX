...
<x-guest-layout>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-8">
        <div class="bg-gray-800 p-6 rounded-lg shadow-lg">
            ...
            <div class="mt-8">
                <h2 class="text-2xl font-bold text-yellow-500 mb-4">Pokemon</h2>
                <ul class="grid sm:grid-cols-2 gap-8">
                    @forelse ($pokemon as $pokemon)
                        <li>
                            <div class="bg-gray-900 rounded-lg shadow-lg overflow-hidden">
                                <div class="h-48 bg-cover bg-center" style="background-image: url('{{ asset('storage/' . $pokemon->image) }}')"></div>
                                <div class="p-4">
                                    <h2 class="text-xl font-bold text-yellow-500">{{ $pokemon->nom }}</h2>
                                    <p class="text-gray-300 text-sm"><strong>PV:</strong> {{ $pokemon->pv }}</p>
                    ...
                    @empty
                        <div class="text-gray-300">Aucun pokemon</div>
                    @endforelse
                </ul>
            </div>
            ...
            <div class="mt-8">
                        ...
                        <div class="flex bg-gray-900 rounded-md shadow p-4 space-x-4">
                            ...
                            <div class="flex flex-col justify-center w-full space-y-4">
                                <div class="flex justify-between">
                                    <div class="flex space-x-4 items-center justify-center">
                                        <div class="flex flex-col justify-center">
                                            <div class="text-gray-300">{{ $commentaire->user->name }}</div>
                                            <div class="text-gray-500 text-sm">{{ $commentaire->created_at->diffForHumans() }}</div>
                                        </div>
                                    </div>
                                    @can('delete', $commentaire)
                                        <div class="flex justify-center">
                                            <button x-data="{ id: {{ $commentaire->id }} }" x-on:click.prevent="window.selected = id; $dispatch('open-modal', 'confirm-comment-deletion');" type="submit" class="font-bold bg-gray-700 text-gray-300 px-4 py-2 rounded shadow">
                                ...
                                <div class="flex flex-col justify-center w-full text-gray-300">
                                    <p class="border bg-gray-800 rounded-md p-4">{{ $commentaire->body }}</p>
                                </div>
                            </div>
                        </div>
                    @empty
                ...
                <x-modal name="confirm-comment-deletion" focusable>
                    <form method="post" onsubmit="event.target.action= '/pokemon/{{ $pokemon->id ?? 1 }}/commentaires/' + window.selected" class="p-6">
                        @csrf @method('DELETE')
