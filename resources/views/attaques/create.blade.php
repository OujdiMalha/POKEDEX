<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 py-12">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            ...
            <form method="POST" action="{{ route('attaques.store') }}" class="flex flex-col space-y-4 text-gray-500">
                @csrf
                <div>
                    <x-input-label for="nom" :value="__('Nom')" />
                    <x-text-input id="nom" class="block mt-1 w-full" type="text" name="nom" :value="old('name')" autofocus />
                    <x-input-error :messages="$errors->get('nom')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="degats" :value="__('Dégâts')" />
                    <x-text-input id="degats" class="block mt-1 w-full" type="number" name="degats" :value="old('degats')" />
                    <x-input-error :messages="$errors->get('degats')" class="mt-2" />
                </div>
                <div>
